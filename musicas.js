const songs=[

/* =========================
   001 — CHUVAS DE GRAÇA
========================= */

{
  id:'001',
  number:'001',
  title:'CHUVAS DE GRAÇA',
  category:'Harpa Cristã',
  sourceToneIndex:10/* índice calculado pelo app a partir do tom Bb */,

  sections:[
    {
      label:'1ª estrofe',
    lines:[
      {lyric:'Deus prometeu com certeza Chuvas de graça mandar;',anchors:[{pos:0,chord:'Bb'},{pos:11,chord:'F'},{pos:22,chord:'Bb'},{pos:29,chord:'F'},{pos:48,chord:'Bb'}]},
      {lyric:'Ele nos dá fortaleza, E ricas bênçãos sem par',anchors:[{pos:0,chord:'Eb'},{pos:17,chord:'Bb'},{pos:24,chord:'Gm'},{pos:33,chord:'C'},{pos:44,chord:'F'}]}
    ]
  },
  {
    label:'Coro',
    lines:[
      {lyric:'Chuvas de graça,',anchors:[{pos:0,chord:'Bb'}]},
      {lyric:'Chuvas pedimos, Senhor;',anchors:[{pos:0,chord:'Bb'},{pos:9,chord:'Gm'},{pos:20,chord:'F'}]},
      {lyric:'Manda-nos chuvas constantes,',anchors:[{pos:0,chord:'Bb'},{pos:11,chord:'Eb'},{pos:22,chord:'Bb'},{pos:26,chord:'Gm'}]},
      {lyric:'Chuvas do Consolador.',anchors:[{pos:1,chord:'F'},{pos:18,chord:'Bb'}]}
    ]
  },
  {
    label:'2ª estrofe',
    lines:[
      {lyric:'Cristo nos tem concedido O santo Consolador,',anchors:[{pos:0,chord:'Bb'},{pos:12,chord:'F'},{pos:21,chord:'Bb'},{pos:27,chord:'F'},{pos:43,chord:'Bb'}]},
      {lyric:'De plena paz nos enchido, Para o reinado de amor.',anchors:[{pos:0,chord:'Eb'},{pos:21,chord:'Bb'},{pos:26,chord:'Gm'},{pos:40,chord:'C'},{pos:48,chord:'F'}]}
    ]
  },
  {
    label:'3ª estrofe',
    lines:[
      {lyric:'Dá-nos, Senhor, amplamente, Teu grande gozo e poder;',anchors:[{pos:0,chord:'Bb'},{pos:11,chord:'F'},{pos:22,chord:'Bb'},{pos:31,chord:'F'},{pos:50,chord:'Bb'}]},
      {lyric:'Fonte de amor permanente, Põe dentro de nosso ser.',anchors:[{pos:0,chord:'Eb'},{pos:21,chord:'Bb'},{pos:26,chord:'Gm'},{pos:40,chord:'C'},{pos:49,chord:'F'}]}
    ]
  },
  {
    label:'4ª estrofe',
    lines:[
      {lyric:'Faze os teus servos piedosos, Dá-lhes virtude e valor,',anchors:[{pos:0,chord:'Bb'},{pos:14,chord:'F'},{pos:23,chord:'Bb'},{pos:33,chord:'F'},{pos:53,chord:'Bb'}]},
      {lyric:'Dando os teus dons preciosos, Do santo Preceptor.',anchors:[{pos:0,chord:'Eb'},{pos:26,chord:'Bb'},{pos:32,chord:'Gm'},{pos:42,chord:'C'},{pos:48,chord:'F'}]}
    ]
  }
]
},

/* =========================
   002 — SAUDOSA LEMBRANÇA
========================= */

 {id:'002',number:'002',title:'SAUDOSA LEMBRANÇA',category:'Harpa Cristã',sourceToneIndex:0,sections:[
  {      label:'1ª estrofe',
      lines:[
        {lyric:'Oh! que saudosa lembrança / Tenho de ti, ó Sião,',anchors:[{pos:0,chord:'C'},{pos:11,chord:'F/G'},{pos:21,chord:'C'},{pos:37,chord:'Am'},{pos:46,chord:'G'}
        ]},
        {lyric:'Terra que eu tanto amo, / Pois és do meu coração.',anchors:[{pos:0,chord:'G'},{pos:47,chord:'C'}
        ]},
        {lyric:'Eu para ti voarei, / Quando o Senhor meu voltar;',anchors:[{pos:0,chord:'C'},{pos:7,chord:'F/G'},{pos:16,chord:'C'},{pos:46,chord:'F'}
        ]},
        {lyric:'Pois Ele foi para o céu, / E breve vem me buscar.',anchors:[{pos:0,chord:'F'},{pos:21,chord:'C'},{pos:27,chord:'G'},{pos:47,chord:'C'}
        ]}
      ]
    },
    {
      label:'Coro',
      lines:[
        {lyric:'Sim, eu porfarei por essa terra de além;',anchors:[{pos:0,chord:'C'},{pos:38,chord:'Bdim/D'}
        ]},
        {lyric:'E lá terminarei as muitas lutas de aquém;',anchors:[{pos:0,chord:'G'},{pos:38,chord:'C'}
        ]},
        {lyric:'Lá está meu bom Senhor, ao qual eu desejo ver;',anchors:[{pos:0,chord:'C'},{pos:43,chord:'F'}
        ]},
        {lyric:'Ele é tudo p\'ra mim, e sem Ele não posso viver.',anchors:[{pos:0,chord:'Dm'},{pos:17,chord:'C'},{pos:27,chord:'Am'},{pos:36,chord:'G'},
          {pos:45,chord:'C'}
        ]}
      ]
    },
    {
      label:'2ª estrofe',
      lines:[
        {lyric:'Bela, mui bela, é a esperança, / Dos que vigiam por ti,',anchors:[{pos:0,chord:'C'},{pos:10,chord:'F/G'},{pos:26,chord:'C'},{pos:43,chord:'Am'},{pos:53,chord:'G'}
        ]},
        {lyric:'Pois eles recebem força, / Que só se encontra ali;',anchors:[{pos:0,chord:'G'},{pos:47,chord:'C'}
        ]},
        {lyric:'Os que procuram chegar / Ao teu regaço, ó Sião,',anchors:[{pos:0,chord:'C'},{pos:10,chord:'F/G'},{pos:20,chord:'C'},{pos:45,chord:'F'}
        ]},
        {lyric:'Livres serão de pecar / E de toda a tentação.',anchors:[{pos:0,chord:'F'},{pos:20,chord:'C'},{pos:25,chord:'G'},{pos:44,chord:'C'}
        ]}
      ]
    },
    {
      label:'3ª estrofe',
      lines:[
        {lyric:'Diz a Sagrada Escritura, / Que são formosos os pés',anchors:[{pos:0,chord:'C'},{pos:10,chord:'F/G'},{pos:19,chord:'C'},{pos:39,chord:'Am'},{pos:48,chord:'G'}
        ]},
        {lyric:'Daqueles que boas novas / Levam para os infiéis;',anchors:[{pos:0,chord:'G'},{pos:45,chord:'C'}
        ]},
        {lyric:'E, se tão belo é falar / Dessas grandezas, aqui,',anchors:[{pos:0,chord:'C'},{pos:10,chord:'F/G'},{pos:21,chord:'C'},{pos:46,chord:'F'}
        ]},
        {lyric:'Que não será o gozar / A graça que existe ali!',anchors:[{pos:0,chord:'F'},{pos:19,chord:'C'},{pos:26,chord:'G'},{pos:43,chord:'C'}
        ]}
      ]
    }
  ]
},

/* =========================
   004 — DEUS VELARÁ POR TI
========================= */

{
  id:'004',
  number:'004',
  title:'DEUS VELARÁ POR TI',
  category:'Harpa Cristã',
  sourceToneIndex:10/* índice calculado pelo app a partir do tom Bb */,

  sections:[
    {
      label:'1ª ESTROFE',
      lines:[
      {lyric:'Não desanimes, Deus proverá; / Deus velará por ti;',anchors:[{pos:0,chord:'Bb'},{pos:9,chord:'F'},{pos:22,chord:'Bb'},{pos:32,chord:'F'},{pos:49,chord:'Bb'}]},
      {lyric:'Sob Suas asas te acolherá; / Deus velará por ti.',anchors:[{pos:0,chord:'Bb'},{pos:10,chord:'F'},{pos:19,chord:'Bb'},{pos:30,chord:'F'},{pos:47,chord:'Bb'}]}
    ]
    },
    {
      label:'CORO',
      lines:[
      {lyric:'Deus cuidará de ti',anchors:[{pos:0,chord:'Eb'},{pos:11,chord:'Eb/Bb'},{pos:17,chord:'Bb'}]},
      {lyric:'No teu viver, no teu sofrer;',anchors:[{pos:0,chord:'F'},{pos:26,chord:'Bb'}]},
      {lyric:'Seu olhar te acompanhará;',anchors:[{pos:0,chord:'Bb'},{pos:11,chord:'Bb/D'},{pos:15,chord:'Eb'},{pos:24,chord:'D'}]},
      {lyric:'Deus velará por ti.',anchors:[{pos:0,chord:'Eb'},{pos:10,chord:'Eb/F'},{pos:14,chord:'F'},{pos:17,chord:'Bb'}]}
    ]
    },
    {
      label:'2ª ESTROFE',
      lines:[
      {lyric:'Se o coração palpitar de dor, / Deus velará por ti;',anchors:[{pos:0,chord:'Bb'},{pos:11,chord:'F'},{pos:20,chord:'Bb'},{pos:33,chord:'F'},{pos:50,chord:'Bb'}]},
      {lyric:'Tu já provaste Seu terno amor. / Deus velará por ti.',anchors:[{pos:0,chord:'Bb'},{pos:11,chord:'F'},{pos:20,chord:'Bb'},{pos:34,chord:'F'},{pos:50,chord:'Bb'}]}
    ]
    },
    {
      label:'3ª ESTROFE',
      lines:[
      {lyric:'Nos desalentos, nas provações, / Deus velará por ti;',anchors:[{pos:0,chord:'Bb'},{pos:9,chord:'F'},{pos:21,chord:'Bb'},{pos:34,chord:'F'},{pos:51,chord:'Bb'}]},
      {lyric:'Lembra-te dEle nas tentações; / Deus velará por ti.',anchors:[{pos:0,chord:'Bb'},{pos:11,chord:'F'},{pos:20,chord:'Bb'},{pos:33,chord:'F'},{pos:50,chord:'Bb'}]}
    ]
    },
    {
      label:'4ª ESTROFE',
      lines:[
      {lyric:'Tudo o que pedes, Ele fará; / Deus velará por ti;',anchors:[{pos:0,chord:'Bb'},{pos:12,chord:'F'},{pos:19,chord:'Bb'},{pos:31,chord:'F'},{pos:48,chord:'Bb'}]},
      {lyric:'E o que precisas, não negará. / Deus velará por ti.',anchors:[{pos:0,chord:'Bb'},{pos:13,chord:'F'},{pos:23,chord:'Bb'},{pos:33,chord:'F'},{pos:50,chord:'Bb'}]}
    ]
    },
    {
      label:'5ª ESTROFE',
      lines:[
      {lyric:'Como estiveres, não temas, vem! / Deus velará por ti;',anchors:[{pos:0,chord:'Bb'},{pos:10,chord:'F'},{pos:21,chord:'Bb'},{pos:35,chord:'F'},{pos:50,chord:'Bb'}]},
      {lyric:'Ele te entende e te ama bem! / Deus velará por ti.',anchors:[{pos:0,chord:'Bb'},{pos:10,chord:'F'},{pos:21,chord:'Bb'},{pos:32,chord:'F'},{pos:49,chord:'Bb'}]}
    ]
    }
  ]
},

/* =========================
   007 — CRISTO CURA, SIM!
========================= */

{
  id:'007',
  number:'007',
  title:'CRISTO CURA, SIM!',
  category:'Harpa Cristã',
  sourceToneIndex:5,

  sections:[
    {
      label:'1ª estrofe',
      lines:[
      {lyric:'Contra os males deste mundo, / Deus nos vale só;',anchors:[{pos:0,chord:'F'},{pos:17,chord:'C'},{pos:32,chord:'F'},{pos:46,chord:'Bb'}]},
      {lyric:'Não há mal que Deus não cure, / Pois de nós tem dó.',anchors:[{pos:0,chord:'Gm'},{pos:35,chord:'C'},{pos:42,chord:'G/B'},{pos:50,chord:'C'}]}
    ]
    },
    {
      label:'Coro',
      lines:[
      {lyric:'Cristo cura, sim,',anchors:[{pos:0,chord:'F'},{pos:9,chord:'C'},{pos:16,chord:'F'}]},
      {lyric:'Cristo cura, sim,',anchors:[{pos:0,chord:'Dm'},{pos:9,chord:'F/A'},{pos:15,chord:'Bb'}]},
      {lyric:'Seu amor por nós é imenso;',anchors:[{pos:0,chord:'F'},{pos:14,chord:'C'},{pos:22,chord:'F'}]},
      {lyric:'Ele cura, sim!',anchors:[{pos:0,chord:'Dm'},{pos:5,chord:'C'},{pos:11,chord:'F'}]}
    ]
    },
    {
      label:'2ª estrofe',
      lines:[
      {lyric:'Derramou Seu sangue puro / P\'ra remir a mim;',anchors:[{pos:0,chord:'F'},{pos:14,chord:'C'},{pos:29,chord:'F'},{pos:42,chord:'Bb'}]},
      {lyric:'Quando ungido sou de azeite, / Sou curado, enfim!',anchors:[{pos:0,chord:'Gm'},{pos:32,chord:'C'},{pos:37,chord:'G/B'},{pos:47,chord:'C'}]}
    ]
    },
    {
      label:'3ª estrofe',
      lines:[
      {lyric:'Só noss\'alma é bem segura, / Oculta em Jesus;',anchors:[{pos:0,chord:'F'},{pos:16,chord:'C'},{pos:30,chord:'F'},{pos:42,chord:'Bb'}]},
      {lyric:'Ele o bálsamo da vida, / Derramou na cruz.',anchors:[{pos:0,chord:'Gm'},{pos:26,chord:'C'},{pos:32,chord:'G/B'},{pos:40,chord:'C'}]}
    ]
    },
    {
      label:'4ª estrofe',
      lines:[
      {lyric:'Glória a Deus! Eterna glória, / Demos-Lhe louvor;',anchors:[{pos:1,chord:'F'},{pos:17,chord:'C'},{pos:33,chord:'F'},{pos:47,chord:'Bb'}]},
      {lyric:'Glória, cânticos e hosanas / Dai ao Redentor!',anchors:[{pos:0,chord:'Gm'},{pos:30,chord:'C'},{pos:36,chord:'G/B'},{pos:43,chord:'C'}]}
    ]
    }
  ]
},

/* =========================
   0015 — CONVERSÃO
========================= */

{
  id:'0015',
  number:'0015',
  title:'CONVERSÃO',
  category:'Harpa Cristã',
  sourceToneIndex:7,

  sections:[
    {
      label:'1ª estrofe',
      lines:[
      {lyric:'Oh! quão cego andei e perdido vaguei,',anchors:[{pos:11,chord:'G'},{pos:27,chord:'D'}]},
      {lyric:'Longe, longe do meu Salvador!',anchors:[{pos:28,chord:'G'}]},
      {lyric:'Mas do céu Ele desceu, e Seu sangue verteu',anchors:[{pos:8,chord:'C'},{pos:31,chord:'G'}]},
      {lyric:'P\'ra salvar um tão pobre pecador.',anchors:[{pos:8,chord:'Am'},{pos:23,chord:'D'},{pos:32,chord:'G'}]}
    ]
    },
    {
      label:'Coro',
      lines:[
      {lyric:'Foi na cruz, foi na cruz,',anchors:[{pos:9,chord:'G'}]},
      {lyric:'Onde um dia eu vi',anchors:[{pos:9,chord:'D'}]},
      {lyric:'Meu pecado castigado em Jesus;',anchors:[{pos:7,chord:'D'},{pos:28,chord:'G'}]},
      {lyric:'Foi ali, pela fé, que os olhos abri,',anchors:[{pos:5,chord:'C'},{pos:27,chord:'G'}]},
      {lyric:'E agora me alegro em Sua luz.',anchors:[{pos:4,chord:'Am'},{pos:13,chord:'D'},{pos:27,chord:'G'}]}
    ]
    },
    {
      label:'2ª estrofe',
      lines:[
      {lyric:'Eu ouvia falar dessa graça sem par,',anchors:[{pos:6,chord:'G'},{pos:23,chord:'D'}]},
      {lyric:'Que do céu trouxe nosso Jesus;',anchors:[{pos:28,chord:'G'}]},
      {lyric:'Mas eu surdo me fiz, converter-me não quis',anchors:[{pos:8,chord:'C'},{pos:29,chord:'G'}]},
      {lyric:'Ao Senhor, que por mim morreu na cruz.',anchors:[{pos:7,chord:'Am'},{pos:21,chord:'D'},{pos:37,chord:'G'}]}
    ]
    },
	{
      label:'3ª estrofe',
      lines:[
      {lyric:'Mas um dia senti meu pecado, e vi ',anchors:[{pos:9,chord:'G'},{pos:33,chord:'D'}]},
      {lyric:'Sobre mim a espada da lei;',anchors:[{pos:25,chord:'G'}]},
      {lyric:'Apressado fugi, em Jesus me escondi, ',anchors:[{pos:10,chord:'C'},{pos:35,chord:'G'}]},
      {lyric:'E abrigo seguro nEle achei.',anchors:[{pos:5,chord:'Am'},{pos:12,chord:'D'},{pos:26,chord:'G'}]}
    ]
    },
    {
      label:'4ª estrofe',
      lines:[
      {lyric:'Quão ditoso, então, este meu coração,',anchors:[{pos:11,chord:'G'},{pos:36,chord:'D'}]},
      {lyric:'Conhecendo o excelso amor',anchors:[{pos:24,chord:'G'}]},
      {lyric:'Que levou meu Jesus a sofrer lá na cruz; ',anchors:[{pos:11,chord:'C'},{pos:39,chord:'G'}]},
      {lyric:'P\'ra salvar a um ·tão pobre pecador.',anchors:[{pos:9,chord:'Am'},{pos:24,chord:'D'},{pos:35,chord:'G'}]}
    ]
    }
  ]
},
/* =========================
   077 — GUARDA O CONTACTO
========================= */

{
  id:'077',
  number:'077',
  title:'GUARDA O CONTACTO',
  category:'Harpa Cristã',
  sourceToneIndex:0,

  sections:[
    {
      label:'1ª estrofe',
      lines:[
      {lyric:'Queres, neste mundo, ser um vencedor?',anchors:[{pos:0,chord:'C'},{pos:35,chord:'Dm'}]},
      {lyric:'Queres tu cantar nas lutas e na dor?',anchors:[{pos:0,chord:'G'},{pos:35,chord:'C'}]},
      {lyric:'Queres ser alegre, qual bom lutador?',anchors:[{pos:0,chord:'C'},{pos:35,chord:'F'}]}
    ]
    },
    {
      label:'Coro',
      lines:[
      {lyric:'Guarda o contacto com teu Salvador!',anchors:[{pos:0,chord:'F'},{pos:10,chord:'C/G'},{pos:20,chord:'G'},{pos:34,chord:'C'}]},
      {lyric:'Guarda o contacto com teu Salvador,',anchors:[{pos:0,chord:'C'},{pos:13,chord:'G'},{pos:34,chord:'C'}]},
      {lyric:'E a nuvem do mal não te cobrirá;',anchors:[{pos:0,chord:'F'},{pos:11,chord:'C'},{pos:25,chord:'D'},{pos:31,chord:'G'}]},
      {lyric:'Pela senda alegre, tu caminharás',anchors:[{pos:0,chord:'C'},{pos:31,chord:'F'}]},
      {lyric:'Indo em contacto com teu Salvador.',anchors:[{pos:0,chord:'F'},{pos:12,chord:'C/G'},{pos:19,chord:'G'},{pos:33,chord:'C'}]}
    ]
    },
    {
      label:'2ª estrofe',
      lines:[
      {lyric:'Neste mundo, vivem muitos a penar,',anchors:[{pos:0,chord:'C'},{pos:32,chord:'Dm'}]},
      {lyric:'Cujos corações transbordam de pesar;',anchors:[{pos:0,chord:'G'},{pos:35,chord:'C'}]},
      {lyric:'Dá-lhes a mensagem de amor sem par;',anchors:[{pos:0,chord:'C'},{pos:34,chord:'F'}]},
      {lyric:'Com Deus o contacto deves tu guardar!',anchors:[{pos:0,chord:'F'},{pos:14,chord:'C/G'},{pos:22,chord:'G'},{pos:36,chord:'C'}]}
    ]
    },
    {
      label:'3ª estrofe',
      lines:[
      {lyric:'Queres tu, com Deus, a comunhão obter?',anchors:[{pos:0,chord:'C'},{pos:36,chord:'Dm'}]},
      {lyric:'Sua glória em ti sempre permanecer?',anchors:[{pos:0,chord:'G'},{pos:34,chord:'C'}]},
      {lyric:'Que o mundo possa Cristo em ti ver!',anchors:[{pos:0,chord:'C'},{pos:34,chord:'F'}]},
      {lyric:'Guarda o contacto co’o supremo Ser.',anchors:[{pos:0,chord:'F'},{pos:13,chord:'C/G'},{pos:20,chord:'G'},{pos:34,chord:'C'}]}
    ]
    },
    {
      label:'4ª estrofe',
      lines:[
      {lyric:'Deixa que o Espírito implante em teu ser,',anchors:[{pos:0,chord:'C'},{pos:39,chord:'Dm'}]},
      {lyric:'O amor de Cristo, divinal prazer;',anchors:[{pos:0,chord:'G'},{pos:32,chord:'C'}]},
      {lyric:'Queres, neste mundo, todo o mal vencer?',anchors:[{pos:0,chord:'C'},{pos:38,chord:'F'}]},
      {lyric:'Guarda o contacto e terás poder!',anchors:[{pos:0,chord:'F'},{pos:12,chord:'C/G'},{pos:18,chord:'G'},{pos:31,chord:'C'}]}
    ]
    }
  ]
},

/* =========================
   198 — JESUS, O BOM AMIGO
========================= */

{
  id:'198',
  number:'198',
  title:'JESUS, O BOM AMIGO',
  category:'Harpa Cristã',
  sourceToneIndex:5,

  sections:[
    {
      label:'1ª estrofe',
      lines:[
      {lyric:'Achei um bom amigo, / Jesus, o Salvador,',anchors:[{pos:0,chord:'F'},{pos:25,chord:'Bb'},{pos:37,chord:'F'}]},
      {lyric:'O escolhido dos milhares para mim;',anchors:[{pos:7,chord:'F'},{pos:31,chord:'C'}]},
      {lyric:'Dos vales é o lírio, é o forte Mediador, / Que me purifica e guarda para Si,',anchors:[{pos:5,chord:'F'},{pos:26,chord:'Bb'},{pos:38,chord:'F'},{pos:52,chord:'F'},{pos:63,chord:'Bb'},{pos:71,chord:'C'},{pos:74,chord:'F'}]},
      {lyric:'Consolador amado, meu protetor do mal, / Solicitude minha toma a Si.',anchors:[{pos:3,chord:'Bb'},{pos:24,chord:'F'},{pos:45,chord:'F'},{pos:66,chord:'C'}]}
    ]
    },
    {
      label:'Coro',
      lines:[
      {lyric:'Dos vales é o lírio, a estrela da manhã,',anchors:[{pos:5,chord:'F'},{pos:25,chord:'Bb'},{pos:38,chord:'F'}]},
      {lyric:'O escolhido dos milhares para mim.',anchors:[{pos:7,chord:'F'},{pos:20,chord:'Bb'},{pos:26,chord:'C'},{pos:31,chord:'F'}]},
      {lyric:'Consolador amado, meu protetor do mal,',anchors:[{pos:3,chord:'Bb'},{pos:24,chord:'F'}]},
      {lyric:'Solicitude minha toma a Si,',anchors:[{pos:2,chord:'F'},{pos:25,chord:'C'}]},
      {lyric:'Dos vales é o lírio, a estrela da manhã,',anchors:[{pos:5,chord:'F'},{pos:25,chord:'Bb'},{pos:38,chord:'F'}]},
      {lyric:'O escolhido dos milhares para mim.',anchors:[{pos:8,chord:'F'},{pos:19,chord:'Bb'},{pos:26,chord:'C'},{pos:32,chord:'F'}]}
    ]
    },
    {
      label:'2ª estrofe',
      lines:[
      {lyric:'Levou-me as dores todas, / As mágoas lhe entreguei;',anchors:[{pos:2,chord:'F'},{pos:32,chord:'Bb'},{pos:48,chord:'F'}]},
      {lyric:'Minha fortaleza é, na tentação.',anchors:[{pos:0,chord:'F'},{pos:28,chord:'C'}]},
      {lyric:'Deixei, por Ele tudo; os ídolos queimei; / Ele me conserva santo o coração,',anchors:[{pos:4,chord:'F'},{pos:25,chord:'Bb'},{pos:38,chord:'F'},{pos:49,chord:'F'},{pos:60,chord:'Bb'},{pos:69,chord:'C'},{pos:74,chord:'F'}]},
      {lyric:'Que o mundo me abandone; persiga o tentador; / Jesus me guarda até da vida o fim.',anchors:[{pos:0,chord:'Bb'},{pos:28,chord:'F'},{pos:49,chord:'F'},{pos:79,chord:'C'}]}
    ]
    },
    {
      label:'3ª estrofe',
      lines:[
      {lyric:'Não desampara nunca, / Nem me abandonará,',anchors:[{pos:4,chord:'F'},{pos:27,chord:'Bb'},{pos:39,chord:'F'}]},
      {lyric:'Se fiel e obediente eu viver;',anchors:[{pos:5,chord:'F'},{pos:27,chord:'C'}]},
      {lyric:'Um muro é de fogo, que me protegerá, / \'Té que venha a mim o tempo de morrer,',anchors:[{pos:4,chord:'F'},{pos:24,chord:'Bb'},{pos:34,chord:'F'},{pos:49,chord:'F'},{pos:62,chord:'Bb'},{pos:68,chord:'C'},{pos:76,chord:'F'}]},
      {lyric:'Ao céu então voando, Sua glória eu verei / Onde a dor e a morte nunca vêm.',anchors:[{pos:3,chord:'Bb'},{pos:27,chord:'F'},{pos:49,chord:'F'},{pos:72,chord:'C'}]}
    ]
    }
  ]
},

/* =========================
   200 — O BONDOSO AMIGO
========================= */

 {id:'200',
  number:'200',
  title:'O BONDOSO AMIGO',
  category:'Harpa Cristã',
  sourceToneIndex:5,

  sections:[
    {
      label:'1ª estrofe',
      lines:[
      {lyric:'Quão bondoso amigo é Cristo! Carregou a nossa dor',anchors:[{pos:0,chord:'F'},{pos:9,chord:'F7'},{pos:22,chord:'Bb'},{pos:30,chord:'F'},{pos:48,chord:'C'}]},
      {lyric:'E nos manda que levemos os cuidados ao Senhor',anchors:[{pos:0,chord:'F'},{pos:7,chord:'F7'},{pos:19,chord:'Bb'},{pos:25,chord:'C'},{pos:44,chord:'F'}]},
      {lyric:'Falta ao coração dorido gozo, paz, consolação?',anchors:[{pos:0,chord:'C7'},{pos:19,chord:'F'},{pos:25,chord:'Bb'},{pos:45,chord:'C'}]},
      {lyric:'Isso é porque não levamos tudo a Deus em oração',anchors:[{pos:0,chord:'F'},{pos:8,chord:'F7'},{pos:21,chord:'Bb'},{pos:46,chord:'F'},{pos:27,chord:'C'}]}
    ]
    },
    {
      label:'2ª estrofe',
      lines:[
      {lyric:'Tu estás fraco e carregado de cuidados e temor',anchors:[{pos:0,chord:'F'},{pos:10,chord:'F7'},{pos:23,chord:'Bb'},{pos:30,chord:'F'},{pos:45,chord:'C'}]},
      {lyric:'A Jesus, refúgio eterno, vai com fé teu mal expor',anchors:[{pos:0,chord:'F'},{pos:9,chord:'F7'},{pos:19,chord:'Bb'},{pos:26,chord:'C'},{pos:48,chord:'F'}]},
      {lyric:'Teus amigos te desprezam? Conta-Lhe isto em oração',anchors:[{pos:0,chord:'C7'},{pos:19,chord:'F'},{pos:27,chord:'Bb'},{pos:49,chord:'C'}]},
      {lyric:'E com seu amor tão terno paz terás no coração',anchors:[{pos:0,chord:'F'},{pos:10,chord:'F7'},{pos:20,chord:'Bb'},{pos:26,chord:'C'},{pos:44,chord:'F'}]}
    ]
    },
    {
      label:'3ª estrofe',
      lines:[
      {lyric:'Cristo é verdadeiro amigo, disto provas nos mostrou',anchors:[{pos:0,chord:'F'},{pos:9,chord:'F7'},{pos:22,chord:'Bb'},{pos:28,chord:'F'},{pos:50,chord:'C'}]},
      {lyric:'Quando pra levar consigo o culpado encarnou',anchors:[{pos:0,chord:'F'},{pos:7,chord:'F7'},{pos:20,chord:'Bb'},{pos:28,chord:'C'},{pos:42,chord:'F'}]},
      {lyric:'Derramou seu sangue puro, nossa mancha pra lavar',anchors:[{pos:0,chord:'C7'},{pos:21,chord:'F'},{pos:27,chord:'Bb'},{pos:47,chord:'C'}]},
      {lyric:'Gozo em vida e no futuro nele podemos alcançar',anchors:[{pos:0,chord:'F'},{pos:7,chord:'F7'},{pos:21,chord:'Bb'},{pos:27,chord:'C'},{pos:46,chord:'F'}]}
    ]
    }
  ]}
];